...
const deleteProveedor = (req, res) => {
    proveedoresDAO.deleteProveedor(req.params.idProveedor, data => {
        try {
            ...
            if (data.affectedRows === 0) throw new Err(`Falló la eliminación del Proveedor: ${req.params.idProveedor}`)
            res.send({
                status: true,
                message: `Eliminación del Proveedor: ${req.params.idProveedor} fue exitosa`
            })
        }
        catch (Err) {
            res.send({
